...
class BarChartRace {
  ...
  updateChart(data, animate = true) {
    ...
    bars.exit()
      .transition()
      .duration(animate ? this.config.options.animation.duration * 0.2 : 0)
      .ease(d3.easeLinear)
      .attr("y", svgHeight)
      .style("opacity", 0)
      .remove();
    ...
    if (animate) {
      ...
      labelsUpdate.transition()
        ...
        .tween("text", function(d) {
          const node = this;
          const currentValue = node._currentValue || 0;
          const i = d3.interpolate(currentValue, d.value);
          node._currentValue = d.value;
          return function(t) {
            node.textContent = `${d.label}: ${d3.format(",")(Math.round(i(t)))}`;
          };
        });
    } else {
      // 立即更新，不使用動畫
